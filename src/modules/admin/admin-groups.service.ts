...
export async function listGroupFeaturesForAdmin(groupId: string): Promise<AdminGroupFeatureRow[]> {
  ...
  return FEATURE_REGISTRY.map((feature) => ({
    ...
    label: feature.label,
    group: feature.group,
    state: byKey.has(feature.key) ? byKey.get(feature.key)! : 'inherit',
  }));
}

/**
 * `enabled: null` clears the override (the key falls back to 'inherit');
 * `enabled: true | false` upserts it. Rejects an unknown key up front, same
 * as `admin.service.ts#updateFeature` — FEATURE_REGISTRY is the source of
...
export async function updateGroupFeatureForAdmin(
  ...
  if (enabled === null) {
    ...
    await upsertGroupFeatureOverride(groupId, key, enabled, adminPhone);
  }
  invalidateGroupOverrideCache();
  await logAdminAction(adminPhone, `PUT /v1/admin/groups/${groupId}/features`, { key, enabled });
