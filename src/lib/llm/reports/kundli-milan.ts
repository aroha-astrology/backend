...
// status) into narrative prose. No fallback filler on a bad response — same
...
const GROUNDING_RULE =
  'The Guna Milan score, Dashakoota score, Manglik status, and compatibility band below are GIVEN FACTS, already computed by a deterministic classical Vedic algorithm. State them verbatim in your prose. Never recompute, second-guess, round differently, or contradict any of these numbers — your job is ONLY to explain what they mean in plain language.';
...
function narrativeSystemPrompt(): string {
  return `You are writing a Kundli Milan (marriage compatibility) report for a mobile astrology app. The app already computed the Guna Milan score, Dashakoota score, and Manglik Dosha status for both people using classical rules. Your job is ONLY to write the narrative explanation.
...
3. Heading close to "Overall Recommendation" — 1 paragraph of balanced, reflective closing guidance that does not overstate certainty either way.
...
function buildFacts(scores: KundliMilanScores): string {
  const lines: string[] = [];
  lines.push(`Guna Milan (Ashtakoota) score: ${scores.gunaMilanScore} out of ${scores.gunaMaxScore}.`);
  ...
  lines.push(`Dashakoota (South Indian) score: ${scores.dashakootaScore} out of ${scores.dashakootaMaxScore}.`);
  lines.push(
...
/**
 * One bounded call — 3 short sections comfortably fit well inside
...
export async function translateKundliMilanNarrative(
  ...
  const parsed = parseSections(raw);
  if (!parsed) {
    throw new Error(`kundli milan translation returned unparseable JSON (target=${targetLanguage})`);
  }
  return parsed;
}
