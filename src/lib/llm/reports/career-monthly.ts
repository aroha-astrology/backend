...
function narrativeSystemPrompt(): string {
  return `You are writing this month's Career Report section for a mobile Vedic astrology app. The app already computed which Mahadasha/Antardasha planetary period rules the given month, a month score, and a tone (challenging/mixed/favorable), based on how that period's ruling planet relates to the 10th house (${HOUSE_SIGNIFICATIONS[10]}) and 6th house (${HOUSE_SIGNIFICATIONS[6]}). Your job is ONLY to write the narrative explanation.
...
2. Heading close to "Practical Guidance" — 1 paragraph of general, practical behavioral framing tied to the tone (e.g. when to push forward vs. consolidate).
...
export async function generateCareerMonthlyNarrative(scores: CareerMonthlyScores): Promise<ReportSection[]> {
...
export async function translateCareerMonthlyNarrative(
  ...
  const parsed = parseSections(raw);
  if (!parsed) {
    throw new Error(`career monthly report translation returned unparseable JSON (target=${targetLanguage})`);
  }
  return parsed;
}
