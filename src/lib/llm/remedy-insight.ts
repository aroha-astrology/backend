...
const SAFETY_RULE =
  'This is traditional Lal Kitab guidance for reflection and everyday practice, never a guarantee about real-world outcomes and never a substitute for the reader\'s own judgement or professional (medical, legal, financial) advice. Use tendency language ("classically associated with", "traditionally considered", "often described as"), never absolute predictions. Never predict death or lifespan (the standing death/suicide policy); every other topic — health, marriage, money — is written normally, in the tendency language above. Do NOT recommend gemstones, expensive purchases, or elaborate rituals — Lal Kitab remedies are deliberately simple, low-cost, everyday actions, and the ones given are the only ones on offer.';
