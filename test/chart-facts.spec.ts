...
function makeChart(): Record<string, unknown> {
  return {
    ...
    planets: [
      ...
      { planet: 'Venus', sign: 'Libra', signIndex: 6, house: 7, longitude: 190, isRetrograde: false },
      { planet: 'Mars', sign: 'Aries', signIndex: 0, house: 1, longitude: 10, isRetrograde: false },
      { planet: 'Jupiter', sign: 'Cancer', signIndex: 3, house: 4, longitude: 100, isRetrograde: false },
    ],
    houses: [
...
describe('getVimshottariDashaFromChart', () => {
  ...
  it('derives the same VimshottariDasha calculateVimshottariDasha would produce from the equivalent birthDate/moonLongitude', () => {
    ...
    expect(result!.mahadashas[0]!.startDate.getTime()).toBe(expected.mahadashas[0]!.startDate.getTime());
  });
  ...
  it('returns null when the chart has no julianDay', () => {
    expect(getVimshottariDashaFromChart({ planets: [{ planet: 'Moon', longitude: 10 }] })).toBeNull();
  });
});
