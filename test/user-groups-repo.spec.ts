...
import { userGroups, userGroupMembers, featureFlagGroupOverrides } from '../src/db/schema.js';
import {
  createGroup,
  listGroups,
...
describe('listGroups', () => {
  it('selects every row from user_groups', async () => {
    const rows = [
      { id: 'g1', name: 'A', description: null, createdAt: new Date(), createdBy: null },
    ];
...
describe('listMembers', () => {
  it('joins to users and returns decrypted phoneE164 alongside displayName/addedAt', async () => {
    ...
    const rows = [{ userId: 'user-1', displayName: 'Asha', phoneE164: 'cipher-abc', addedAt }];
    const { chain, calls } = makeSelectChain(rows);
    state.select.mockReturnValue(chain);
...
describe('upsertGroupFeatureOverride', () => {
  it('inserts with ON CONFLICT(group_id, feature_key) DO UPDATE', async () => {
    ...
    expect(target).toEqual([
      featureFlagGroupOverrides.groupId,
      featureFlagGroupOverrides.featureKey,
    ]);
  });
});
