...
describe('computeFinanceMonthlyScores', () => {
  it('reports the correct key houses (2nd accumulated wealth + 11th monthly gains)', () => {
    const scores = computeFinanceMonthlyScores({ chart: makeChart(), partnerChart: null }, '2027-01');
  ...
  it('never throws and degrades gracefully when periodMonth is null', () => {
    expect(() => computeFinanceMonthlyScores({ chart: makeChart(), partnerChart: null }, null)).not.toThrow();
  ...
  it('never throws on a null chart', () => {
    expect(() => computeFinanceMonthlyScores({ chart: null, partnerChart: null }, '2027-01')).not.toThrow();
  });
});
