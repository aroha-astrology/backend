...
const KEMDRUM_PLANETS = ['Mars', 'Mercury', 'Jupiter', 'Venus', 'Saturn'];
function getHouseFromSign(planetSignIndex, referenceSignIndex) {
    return ((planetSignIndex - referenceSignIndex + 12) % 12) + 1;
}
function isAspectedByJupiter(chartData, targetSignIndex) {
...
function detectKemDrumaDosha(chartData) {
    ...
    const hasAdjacentPlanet = chartData.planets.some((p) => {
        if (!KEMDRUM_PLANETS.includes(p.planet))
            return false;
    ...
    if ([1, 4, 7, 10].includes(moonHouseFromLagna)) {
        cancellations.push(`Moon in Kendra (house ${moonHouseFromLagna}) from Lagna - cancellation applies`);
    }
    // 2. Moon aspected by Jupiter
    ...
    // 3. Moon conjunct Venus (same sign)
    const venusConjunct = chartData.planets.some((p) => p.planet === 'Venus' && p.signIndex === moonSignIndex);
    if (venusConjunct) {
...
//# sourceMappingURL=kemDrumaDosha.js.map