...
function getHouseFromSign(planetSignIndex, referenceSignIndex) {
...
function getPlanetsInHouseFromSign(chartData, referenceSignIndex, house) {
    return chartData.planets
        .filter((p) => getHouseFromSign(p.signIndex, referenceSignIndex) === house)
        .map((p) => p.planet);
}
function isAspectedBy(chartData, targetSignIndex, aspectingPlanet) {
...
function checkCancellations(chartData) {
    const cancellations = [];
    ...
    const planetsInMarsSign = chartData.planets.filter((p) => p.signIndex === marsSignIndex && p.planet !== 'Mars');
    const beneficConjunctions = planetsInMarsSign.filter((p) => p.planet === 'Jupiter' || p.planet === 'Venus');
    ...
    if (jupiter) {
        ...
        if ([1, 4, 7, 10].includes(marsHouseFromJupiter)) {
            cancellations.push(`Mars in Kendra (house ${marsHouseFromJupiter}) from Jupiter - cancellation applies`);
        }
    }
...
function detectMangalDosha(chartData) {
    ...
    if (present && cancellations.length === 0) {
        ...
        if (cancellations.length >= 3)
            severity = 'none';
        else if (afflictedCount === 3 && cancellations.length < 3)
            severity = 'moderate';
        else if (afflictedCount === 2 && cancellations.length < 2)
            severity = 'mild';
        else
            severity = 'mild';
    }
