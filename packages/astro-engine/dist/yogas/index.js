...
function isInKendraOrTrikona(house) {
...
function getHouseLord(houseNum, chartData) {
 ...
 */
function doesPlanetAspect(planet, targetHouse, chartData) {
    ...
    if (dist === 7)
        return true;
    // Special aspects
    if (planet === 'Mars' && (dist === 4 || dist === 8))
        return true;
    if (planet === 'Jupiter' && (dist === 5 || dist === 9))
        return true;
    if (planet === 'Saturn' && (dist === 3 || dist === 10))
        return true;
    return false;
}
/** Is a planet aspected by a specific other planet? */
function isPlanetAspectedBy(target, aspector, chartData) {
...
function detectMahapurushaYoga(planet, yogaName, description, chartData) {
...
function detectRuchaka(chartData) {
    return detectMahapurushaYoga('Mars', 'Ruchaka Yoga', 'Mars in own or exalted sign in a Kendra house. Grants courage, leadership, authority, and physical prowess.', chartData);
}
function detectBhadra(chartData) {
    return detectMahapurushaYoga('Mercury', 'Bhadra Yoga', 'Mercury in own or exalted sign in a Kendra house. Grants sharp intellect, eloquence, business acumen, and youthful appearance.', chartData);
}
function detectHamsa(chartData) {
    return detectMahapurushaYoga('Jupiter', 'Hamsa Yoga', 'Jupiter in own or exalted sign in a Kendra house. Grants wisdom, spirituality, morality, and high social status.', chartData);
}
function detectMalavya(chartData) {
    return detectMahapurushaYoga('Venus', 'Malavya Yoga', 'Venus in own or exalted sign in a Kendra house. Grants beauty, wealth, artistic talents, luxurious life, and a loving spouse.', chartData);
}
function detectShasha(chartData) {
    return detectMahapurushaYoga('Saturn', 'Shasha Yoga', 'Saturn in own or exalted sign in a Kendra house. Grants discipline, authority over people, political power, and longevity.', chartData);
}
// ------ Raja Yogas ------
function detectDharmaKarmadhipati(chartData) {
    ...
    if (present) {
        ...
        if (exchange)
            strength = Math.min(100, strength + 10);
        if (conjunct && isInKendraOrTrikona(house9))
            strength = Math.min(100, strength + 5);
    }
    const planets = lord9 === lord10 ? [lord9] : [lord9, lord10];
    const houses = [];
    if (present) {
        houses.push(house9);
        if (house10 !== house9)
    ...
    return {
        ...
        present,
        strength,
        description: '9th lord (dharma) and 10th lord (karma) are connected by conjunction, mutual exchange, or mutual aspect. One of the most powerful Raja Yogas bestowing fortune, fame, and high position.',
        planets,
        houses,
        activationPeriod: `${lord9} or ${lord10} Mahadasha/Antardasha`,
    };
}
function detectViparitaRaja(chartData) {
    ...
    return {
        ...
        present,
        strength,
        description: 'Lords of 6th, 8th, and 12th houses placed in dusthana houses (6, 8, 12). Turns adversity into advantage; gains through unexpected events, enemies\' losses, or insurance.',
...
function detectNeechBhangaRaja(chartData) {
    const involvedPlanets = [];
    const involvedHouses = [];
    let maxStrength = 0;
    for (const pp of chartData.planets) {
        const { planet, sign, house } = pp;
        if (!isPlanetDebilitated(planet, sign))
            continue;
        let cancelled = false;
    ...
    return {
        ...
        description: 'A debilitated planet whose debilitation is cancelled by specific conditions. Transforms weakness into extraordinary strength, often giving rise after initial struggles.',
        planets: involvedPlanets,
        houses: involvedHouses,
        activationPeriod: involvedPlanets.length > 0 ? `${involvedPlanets[0]} Mahadasha/Antardasha` : undefined,
    };
}
function detectMahaBhagya(chartData) {
    // MahaBhagya Yoga:
...
// ------ Dhana Yogas ------
function detectLakshmi(chartData) {
    const lord9 = getHouseLord(9, chartData);
    ...
    const planets = lord9 === 'Venus' ? ['Venus'] : [lord9, 'Venus'];
    return {
        name: 'Lakshmi Yoga',
        type: 'dhana',
        present,
        strength,
        description: '9th lord strong in Kendra/Trikona and Venus in own/exalted sign in Kendra/Trikona. Bestows immense wealth, prosperity, and the blessings of Goddess Lakshmi.',
        planets,
        houses: present ? [lord9House, venusHouse] : [],
        activationPeriod: `Venus or ${lord9} Mahadasha/Antardasha`,
    };
}
function detectVasumathi(chartData) {
    const moonHouse = getPlanetHouse('Moon', chartData);
    if (moonHouse === 0) {
    ...
    for (const benefic of shared_1.NATURAL_BENEFICS) {
        ...
        if (targetHouses.includes(bHouse)) {
            ...
            if (!housesFound.includes(bHouse))
    ...
    let strength = 0;
    if (present) {
        strength = Math.round(beneficsFound.reduce((sum, p) => sum + computePlanetStrength(p, chartData), 0) / beneficsFound.length);
        if (fullPositionsCovered === 4)
            strength = Math.min(100, strength + 10);
    }
    return {
        name: 'Vasumathi Yoga',
        ...
        present,
        strength,
        description: 'Natural benefics occupy the 3rd, 6th, 10th, and 11th houses from Moon. Bestows great wealth accumulated through one\'s own efforts.',
        planets: beneficsFound,
        houses: housesFound,
        activationPeriod: beneficsFound.length > 0 ? `${beneficsFound[0]} Mahadasha/Antardasha` : undefined,
    };
}
function detectKalanidhi(chartData) {
    ...
    const mercuryConjOrAspect = arePlanetsConjunct('Jupiter', 'Mercury', chartData) ||
        isPlanetAspectedBy('Jupiter', 'Mercury', chartData);
    const venusConjOrAspect = arePlanetsConjunct('Jupiter', 'Venus', chartData) ||
    ...
    return {
        ...
        present,
        strength,
        description: 'Jupiter in 2nd or 5th house, conjunct or aspected by Mercury and Venus. Grants immense wealth, artistic talent, knowledge, and cultural refinement.',
...
// ------ Lunar Yogas ------
function detectGajakesari(chartData) {
    const moonHouse = getPlanetHouse('Moon', chartData);
    const jupiterHouse = getPlanetHouse('Jupiter', chartData);
    ...
    return {
        ...
        present,
        strength,
        description: 'Jupiter in a Kendra (1, 4, 7, 10) from Moon. One of the most celebrated yogas; bestows wisdom, eloquence, lasting fame, and prosperity.',
        planets: ['Moon', 'Jupiter'],
        houses: [moonHouse, jupiterHouse],
        activationPeriod: 'Jupiter or Moon Mahadasha/Antardasha',
    };
}
function detectSunapha(chartData) {
    const moonHouse = getPlanetHouse('Moon', chartData);
    if (moonHouse === 0) {
    ...
    const eligible = ['Moon', 'Mars', 'Mercury', 'Jupiter', 'Venus', 'Saturn'];
    ...
    let strength = 0;
    if (present) {
        strength = Math.round(found.reduce((sum, p) => sum + computePlanetStrength(p, chartData), 0) / found.length);
    }
    return {
        name: 'Sunapha Yoga',
        type: 'lunar',
        present,
        strength,
        description: 'Planet(s) other than Sun, Rahu, Ketu in the 2nd house from Moon. Grants self-acquired wealth, intellect, and royal status.',
        planets: ['Moon', ...found],
        houses: present ? [moonHouse, secondFromMoon] : [],
        activationPeriod: found.length > 0 ? `${found[0]} Mahadasha/Antardasha` : undefined,
    };
}
function detectAnapha(chartData) {
    const moonHouse = getPlanetHouse('Moon', chartData);
    if (moonHouse === 0) {
    ...
    const eligible = ['Mars', 'Mercury', 'Jupiter', 'Venus', 'Saturn'];
    const found = [];
    for (const p of eligible) {
    ...
    return {
        ...
        present,
        strength,
        description: 'Planet(s) other than Sun, Rahu, Ketu in the 12th house from Moon. Grants good health, noble character, and fame.',
        planets: ['Moon', ...found],
        houses: present ? [moonHouse, twelfthFromMoon] : [],
        activationPeriod: found.length > 0 ? `${found[0]} Mahadasha/Antardasha` : undefined,
    };
}
function detectDurudhara(chartData) {
    const moonHouse = getPlanetHouse('Moon', chartData);
    if (moonHouse === 0) {
    ...
    let strength = 0;
    if (present) {
        strength = Math.round(allFound.reduce((sum, p) => sum + computePlanetStrength(p, chartData), 0) / allFound.length);
    }
    return {
        name: 'Durudhara Yoga',
        type: 'lunar',
        present,
        strength,
        description: 'Planets in both 2nd and 12th from Moon (flanking the Moon). Grants wealth, vehicles, property, generous nature, and fame.',
...
function detectChandraMangal(chartData) {
    ...
    return {
        ...
        present: conjunct,
        strength,
        description: 'Moon and Mars conjunction (same house). Grants wealth through self-effort, business acumen, and enterprising nature. Can also indicate a sharp temper.',
        planets: ['Moon', 'Mars'],
        houses: conjunct ? [moonHouse] : [],
        activationPeriod: 'Moon or Mars Mahadasha/Antardasha',
    };
}
function detectAdhi(chartData) {
    const moonHouse = getPlanetHouse('Moon', chartData);
    if (moonHouse === 0) {
    ...
    return {
        ...
        present,
        strength,
        description: 'Natural benefics in 6th, 7th, and 8th from Moon. Grants leadership, ministerial position, affluence, and ability to overcome enemies.',
        planets: ['Moon', ...found],
...
// ------ Solar Yogas ------
function detectBudhaditya(chartData) {
    ...
    if (present) {
        ...
        if (mercPos && sunPos) {
            ...
            if (angDist < 3)
                strength = Math.max(10, strength - 15); // Very close combustion
        }
    }
    return {
        ...
        present,
        strength,
        description: 'Sun and Mercury conjunction in a Kendra or Trikona house. Grants intelligence, fame, analytical ability, and success in education and communication.',
        planets: ['Sun', 'Mercury'],
        houses: present ? [sunHouse] : [],
        activationPeriod: 'Sun or Mercury Mahadasha/Antardasha',
    };
}
function detectVeshi(chartData) {
    const sunHouse = getPlanetHouse('Sun', chartData);
    if (sunHouse === 0) {
    ...
    return {
        ...
        present,
        strength,
        description: 'Planet(s) other than Moon, Rahu, Ketu in the 2nd house from Sun. Grants eloquence, wealth, and a respectable position.',
        planets: ['Sun', ...found],
        houses: present ? [sunHouse, secondFromSun] : [],
        activationPeriod: found.length > 0 ? `${found[0]} Mahadasha/Antardasha` : undefined,
    };
}
function detectVoshi(chartData) {
    const sunHouse = getPlanetHouse('Sun', chartData);
    if (sunHouse === 0) {
    ...
    return {
        ...
        present,
        strength,
        description: 'Planet(s) other than Moon, Rahu, Ketu in the 12th house from Sun. Grants charitable nature, learning, and a good reputation.',
        planets: ['Sun', ...found],
        houses: present ? [sunHouse, twelfthFromSun] : [],
        activationPeriod: found.length > 0 ? `${found[0]} Mahadasha/Antardasha` : undefined,
    };
}
function detectUbhayachari(chartData) {
    const sunHouse = getPlanetHouse('Sun', chartData);
    if (sunHouse === 0) {
    ...
    return {
        ...
        present,
        strength,
        description: 'Planets in both 2nd and 12th from Sun (flanking the Sun). Grants royalty, fame, balanced temperament, and all-round prosperity.',
...
// ------ Other Yogas ------
function detectSaraswati(chartData) {
    ...
    return {
        ...
        present,
        strength,
        description: 'Jupiter, Venus, Mercury in Kendra, Trikona, or 2nd house, with Jupiter in own/exalted sign. Grants exceptional learning, wisdom, mastery of arts and sciences.',
        planets: ['Jupiter', 'Venus', 'Mercury'],
        houses: present ? [jupHouse, venHouse, merHouse] : [],
        activationPeriod: 'Jupiter Mahadasha/Antardasha',
    };
}
function detectAmala(chartData) {
    ...
    return {
        ...
        present,
        strength,
        description: 'Natural benefic in the 10th house from Lagna or Moon. Grants a spotless reputation, fame through virtuous deeds, and a charitable nature.',
        planets: found,
        houses: housesFound,
        activationPeriod: found.length > 0 ? `${found[0]} Mahadasha/Antardasha` : undefined,
    };
}
function detectGuruMangala(chartData) {
    ...
    if (conjunct) {
        ...
        if (isInKendraOrTrikona(jupHouse))
            strength = Math.min(100, strength + 10);
    }
    return {
        ...
        present: conjunct,
        strength,
        description: 'Jupiter and Mars conjunction. Grants courage, righteous action, leadership in spiritual or social causes, and success in competitive endeavors.',
        planets: ['Jupiter', 'Mars'],
        houses: conjunct ? [jupHouse] : [],
        activationPeriod: 'Jupiter or Mars Mahadasha/Antardasha',
    };
}
function detectHarsha(chartData) {
    const lord6 = getHouseLord(6, chartData);
    ...
    return {
        ...
        present,
        strength,
        description: '6th lord in the 6th house. Grants good health, ability to defeat enemies, and happiness. One of the three Parijata-group yogas.',
        planets: [lord6],
        houses: present ? [6] : [],
        activationPeriod: `${lord6} Mahadasha/Antardasha`,
    };
}
function detectSarala(chartData) {
    const lord8 = getHouseLord(8, chartData);
    ...
    return {
        ...
        present,
        strength,
        description: '8th lord in the 8th house. Grants longevity, fearlessness, prosperity, and fortune from unexpected sources.',
        planets: [lord8],
        houses: present ? [8] : [],
        activationPeriod: `${lord8} Mahadasha/Antardasha`,
    };
}
function detectVimala(chartData) {
    const lord12 = getHouseLord(12, chartData);
    ...
    return {
        ...
        present,
        strength,
        description: '12th lord in the 12th house. Grants frugal nature, spiritual inclination, minimal expenses, and moksha.',
        planets: [lord12],
        houses: present ? [12] : [],
        activationPeriod: `${lord12} Mahadasha/Antardasha`,
    };
}
function detectChandraAdhi(chartData) {
    ...
    return {
        ...
        present,
        strength,
        description: 'Natural benefics in all of 6th, 7th, and 8th houses from Moon. Full Chandra Adhi Yoga grants commander/king-like status and immense prosperity.',
        planets: ['Moon', ...found],
        houses: [moonHouse, ...housesFound],
        activationPeriod: found.length > 0 ? `${found[0]} Mahadasha/Antardasha` : undefined,
    };
}
function detectKemadrumaCanellation(chartData) {
    ...
    return {
        ...
        present,
        strength,
        description: 'Kemadruma dosha is cancelled because Moon is in a Kendra or aspected by Jupiter. Restores the native\'s fortune, status, and mental peace.',
        planets,
        houses: present ? [moonHouse] : [],
        activationPeriod: 'Moon Mahadasha/Antardasha',
    };
}
function detectParivartana(chartData) {
    const yogas = [];
    ...
    for (let h1 = 1; h1 <= 12; h1++) {
        for (let h2 = h1 + 1; h2 <= 12; h2++) {
            ...
            if (lord1 === lord2)
                continue; // Same lord rules both houses
...
function detectNipuna(chartData) {
    const conjunct = arePlanetsConjunct('Sun', 'Mercury', chartData);
    const sunHouse = getPlanetHouse('Sun', chartData);
    ...
    return {
        ...
        present,
        strength,
        description: 'Mercury and Sun conjunction in a Kendra house. Grants exceptional intelligence, skill in multiple disciplines, and expertise in communication.',
        planets: ['Sun', 'Mercury'],
        houses: present ? [sunHouse] : [],
...
function detectChamara(chartData) {
    ...
    return {
        ...
        present,
        strength,
        description: 'Lagna lord exalted in a Kendra and aspected by Jupiter. Grants learning, royal favor, eloquence, and long life.',
        planets: [lagnaLord, 'Jupiter'],
        houses: present ? [lagnaLordHouse] : [],
        activationPeriod: `${lagnaLord} Mahadasha/Antardasha`,
    };
}
function detectAkhandSamrajya(chartData) {
    ...
    return {
        ...
        present,
        strength,
        description: 'Jupiter lords 2nd, 5th, or 11th house and is in Kendra; lords of 2nd, 9th, 11th are strong. Grants undivided empire, lasting authority, and great wealth.',
        planets: ['Jupiter', lord9],
        houses: present ? [jupHouse] : [],
        activationPeriod: 'Jupiter Mahadasha/Antardasha',
    };
}
function detectKahala(chartData) {
    ...
    return {
        ...
        present,
        strength,
        description: 'Lords of 4th and 9th in mutual Kendras. Grants courage, boldness, leadership in community, and land/property.',
...
function detectParvata(chartData) {
    ...
    return {
        ...
        present,
        strength,
        description: 'Lords of 1st and 12th in mutual Kendras with no malefics in Kendra houses. Grants fame, government favor, and mountain-like stability.',
        planets: lord1 === lord12 ? [lord1] : [lord1, lord12],
        houses: present ? [lord1House, lord12House] : [],
        activationPeriod: `${lord1} Mahadasha/Antardasha`,
    };
}
function detectShankha(chartData) {
    ...
    return {
        ...
        present,
        strength,
        description: 'Lords of 5th and 6th in mutual Kendras with strong Lagna lord. Grants love of life, good morals, longevity, and a comfortable existence.',
...
function detectKhadrga(chartData) {
    ...
    let strength = 0;
    if (present) {
        strength = Math.round((computePlanetStrength(lord9, chartData) +
...
function detectKedarNath(chartData) {
    ...
    return {
        ...
        present,
        strength,
        description: 'All seven planets occupy exactly four houses. Grants agricultural wealth, land ownership, and a helpful, charitable nature.',
        planets: SEVEN_PLANETS,
        houses: present ? [...housesOccupied] : [],
    };
}
function detectMusala(chartData) {
    ...
    return {
        ...
        present: allInFixed,
        strength,
        description: 'All planets in fixed signs (Taurus, Leo, Scorpio, Aquarius). Grants wealth, fame, steadfastness, and a proud nature.',
        planets: SEVEN_PLANETS,
        houses: allInFixed ? SEVEN_PLANETS.map((p) => getPlanetHouse(p, chartData)) : [],
    };
}
function detectNala(chartData) {
    ...
    return {
        ...
        strength,
        description: 'All planets in movable signs (Aries, Cancer, Libra, Capricorn). Grants a dynamic, active nature with fluctuating wealth.',
        planets: SEVEN_PLANETS,
        houses: allInMovable ? SEVEN_PLANETS.map((p) => getPlanetHouse(p, chartData)) : [],
    };
}
function detectRajju(chartData) {
    ...
    return {
        ...
        present: allInDual,
        strength,
        description: 'All planets in dual signs (Gemini, Virgo, Sagittarius, Pisces). Grants love of travel, fondness for foreign lands, and a flexible nature.',
        planets: SEVEN_PLANETS,
        houses: allInDual ? SEVEN_PLANETS.map((p) => getPlanetHouse(p, chartData)) : [],
    };
}
function detectSunapha2ndLord(chartData) {
    ...
    return {
        ...
        present,
        strength,
        description: 'Lords of 2nd and 11th houses in mutual Kendras. Indicates financial prosperity and multiple income sources.',
...
function detectChandraMangalDhana(chartData) {
    ...
    return {
        ...
        present,
        strength,
        description: 'Lords of 1st and 2nd houses conjunct or in mutual exchange. Grants wealth through personal effort and family resources.',
        planets: lord1 === lord2 ? [lord1] : [lord1, lord2],
        houses: present ? [lord1House] : [],
        activationPeriod: `${lord1} or ${lord2} Mahadasha/Antardasha`,
    };
}
function detectGauri(chartData) {
    ...
    return {
        ...
        present,
        strength,
        description: 'Moon in own/exalted sign in Kendra/Trikona aspected by Jupiter. Grants beauty, virtuous spouse, and domestic happiness.',
        planets: ['Moon', 'Jupiter'],
        houses: present ? [moonHouse] : [],
        activationPeriod: 'Moon Mahadasha/Antardasha',
    };
}
function detectBheri(chartData) {
    ...
    const planets = [...new Set(['Venus', 'Jupiter', lord1, lord9])];
    return {
        name: 'Bheri Yoga',
        type: 'benefic',
        present,
        strength,
        description: 'Venus, Jupiter, and Lagna lord in mutual Kendras with strong 9th lord. Grants piety, wealth, happiness, and a long life.',
        planets,
        houses: present ? [venHouse, jupHouse, lord1House] : [],
        activationPeriod: 'Venus or Jupiter Mahadasha/Antardasha',
    };
}
function detectKendradhipatiDosha(chartData) {
    ...
    if (present) {
        ...
        strength = Math.round(uniquePlanets.reduce((sum, p) => sum + computePlanetStrength(p, chartData), 0) / uniquePlanets.length);
    ...
    return {
        ...
        present,
        strength,
        description: 'Natural benefic planets owning Kendra houses lose their beneficence. Indicates reduced positive results from those benefics unless they also lord a trikona.',
        planets: uniquePlanets,
        houses: [...new Set(housesFound)],
    };
}
function detectShadashtak(chartData) {
    ...
    return {
        ...
        present,
        strength,
        description: 'Sun and Saturn in 6-8 relationship (Shadashtak). Indicates tension between authority and discipline, possible conflicts with father/government.',
        planets: ['Sun', 'Saturn'],
        houses: present ? [sunHouse, satHouse] : [],
    };
}
function detectAngarak(chartData) {
    ...
    return {
        ...
        present: conjunct,
        strength,
        description: 'Mars conjunct Rahu. Indicates aggression, accidents, conflicts, and legal troubles. Can also give technical/surgical skills if well-placed.',
        planets: ['Mars', 'Rahu'],
        houses: conjunct ? [marsHouse] : [],
        activationPeriod: 'Mars or Rahu Mahadasha/Antardasha',
    };
}
function detectGuruChandal(chartData) {
    ...
    const jupHouse = getPlanetHouse('Jupiter', chartData);
    const shadowPlanet = conjRahu ? 'Rahu' : 'Ketu';
    let strength = 0;
    if (present) {
        strength = 55;
    ...
    return {
        ...
        present,
        strength,
        description: 'Jupiter conjunct Rahu or Ketu. Corrupts Jupiter\'s wisdom, causing unorthodox beliefs, broken promises, and challenges with gurus/teachers. Can give unconventional success.',
        planets: ['Jupiter', shadowPlanet],
        houses: present ? [jupHouse] : [],
        activationPeriod: `Jupiter or ${shadowPlanet} Mahadasha/Antardasha`,
    };
}
function detectGrahan(chartData) {
...
function detectShakat(chartData) {
    ...
    return {
        ...
        present,
        strength,
        description: 'Moon in 6th, 8th, or 12th from Jupiter. Indicates fluctuating fortune, periods of poverty followed by prosperity, and instability.',
        planets: ['Moon', 'Jupiter'],
        houses: present ? [moonHouse, jupHouse] : [],
    };
}
function detectDaridra(chartData) {
    ...
    return {
        ...
        present,
        strength,
        description: '11th lord in a dusthana house (6, 8, or 12). Indicates financial difficulties, blocked income, and challenges in fulfilling desires.',
        planets: [lord11],
        houses: present ? [lord11House] : [],
        activationPeriod: `${lord11} Mahadasha/Antardasha`,
    };
}
function detectRajaLakshmana(chartData) {
    ...
    for (const kh of kendraHouses) {
        for (const th of trikonaHouses) {
            ...
            const mutualAspect = doesPlanetAspect(kendraLord, tHouse, chartData) &&
...
function detectAllRajaYogas(chartData) {
    const kendraHouses = [1, 4, 7, 10];
...
function detectChaturSagara(chartData) {
    ...
    let strength = 0;
    if (allOccupied) {
        strength = Math.round(planetsInKendras.reduce((sum, p) => sum + computePlanetStrength(p, chartData), 0) /
    ...
    return {
        ...
        present: allOccupied,
        strength,
        description: 'All four Kendra houses (1, 4, 7, 10) are occupied by planets. Grants fame, dominion, long life, and respect from many people.',
...
function detectVasuki(chartData) {
    ...
    return {
        ...
        present,
        strength,
        description: 'Two or more benefics in the 3rd house. Grants courage, artistic skills, and success through self-effort and siblings.',
        planets: found,
        houses: present ? [3] : [],
    };
}
function detectMahaLakshmi(chartData) {
    ...
    const planets = lord5 === lord9 ? [lord5] : [lord5, lord9];
    return {
        name: 'Maha Lakshmi Yoga',
        type: 'dhana',
        present,
        strength,
        description: 'Lords of 5th and 9th in Kendra houses with good strength. Bestows great wealth, fortune, and divine grace.',
        planets,
        houses: present ? [lord5House, lord9House] : [],
        activationPeriod: `${lord5} or ${lord9} Mahadasha/Antardasha`,
    };
}
function detectSreenath(chartData) {
    ...
    return {
        ...
        present,
        strength,
        description: '7th lord in 10th and 10th lord in 7th (mutual exchange). Grants high status through partnerships, marriage, or business associations.',
        planets: lord7 === lord10 ? [lord7] : [lord7, lord10],
        houses: present ? [7, 10] : [],
        activationPeriod: `${lord7} or ${lord10} Mahadasha/Antardasha`,
    };
}
function detectAdhiYogaFromLagna(chartData) {
    ...
    return {
        ...
        present,
        strength,
        description: 'Natural benefics in 6th, 7th, 8th from Lagna. Grants ministerial position, leadership, and authority.',
        planets: found,
        houses: housesFound,
        activationPeriod: found.length > 0 ? `${found[0]} Mahadasha/Antardasha` : undefined,
    };
