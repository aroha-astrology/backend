...
export async function deductCredits(
  ...
  if (error) {
    const isInsufficient = error.message?.includes('INSUFFICIENT_TOKENS') || error.code === 'P0001';
    return {
      success: false,
...
export async function refundCredits(
  ...
  try {
    ...
    if (insertError) {
      console.warn(
        `[refundCredits] audit row failed (refund itself succeeded): ${insertError.message}`,
      );
    }
  } catch (err) {
