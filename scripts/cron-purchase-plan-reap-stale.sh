#!/usr/bin/env bash
#
# Reaps purchase_plans rows stuck at 'processing' because the process that
# claimed them crashed mid-run (see PURCHASE_PLAN_STALE_PROCESSING_MS in purchase-plan.repo.ts
# and reapStaleProcessingPlans in purchase-plan.service.ts).
#
# Wired into the EC2 crontab to run every 5 minutes:
#   */5 * * * * /home/ec2-user/aroha-backend/scripts/cron-purchase-plan-reap-stale.sh \
#     >> /home/ec2-user/cron-purchase-plan-reap-stale.log 2>&1
#
...
curl -fsS --max-time 30 -X POST \
  ...
  -d '{}' \
  "http://127.0.0.1:${PORT}/internal/cron/purchase-plan-reap-stale" || CURL_EXIT=$?

if [ "$CURL_EXIT" -ne 0 ]; then
  echo "$(date -u +%FT%TZ) ERROR: cron-purchase-plan-reap-stale.sh failed (curl exit $CURL_EXIT)" >&2
  if [ -n "$TELEGRAM_BOT_TOKEN" ] && [ -n "$TELEGRAM_ALERT_CHAT_ID" ]; then
    curl -fsS --max-time 10 -X POST \
      -H 'Content-Type: application/json' \
      -d "{\"chat_id\":\"${TELEGRAM_ALERT_CHAT_ID}\",\"text\":\"cron-purchase-plan-reap-stale.sh failed (curl exit ${CURL_EXIT})\"}" \
