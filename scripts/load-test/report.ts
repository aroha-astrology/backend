...
const steps: { file: string; summary: StepSummary }[] = summaryFiles
  .map((f) => ({
    file: f,
    summary: JSON.parse(readFileSync(join(runDir, f), 'utf8')) as StepSummary,
  }))
...
const lastClean = [...ramp]
  .reverse()
  .find((s) => !s.summary.aborted && breachedThresholds(s.summary).length === 0);
...
if (lastClean) {
  md +=
    `**Last clean step:** ${lastClean.summary.concurrency} concurrent VUs — ` +
...
if (firstBreach) {
  ...
  md +=
    `**First breach:** ${firstBreach.summary.concurrency} concurrent VUs` +
    (firstBreach.summary.aborted
      ? ' — monitor aborted the run'
      : ` — ${reasons.join(', ')} exceeded threshold`) +
    `.\n\n`;
} else {
...
if (plateau) {
  ...
  if (plateauMetrics.length > 0) {
    ...
    md +=
      `CPU steal — first third avg ${avgSteal(early).toFixed(1)}%, last third avg ${avgSteal(late).toFixed(1)}%` +
...
if (capacitySource) {
  ...
  md +=
    `Sustained ${rps.toFixed(1)} RPS ÷ ${callsPerOpen} calls/app-open ≈ **${sessionsPerSec.toFixed(1)} concurrent app-opens/sec** ` +
...
md += `## Notes\n\n`;
md +=
  `- Abort thresholds used: error rate > ${(ABORT_THRESHOLDS.nonRateLimitErrorRate * 100).toFixed(0)}%, ` +
