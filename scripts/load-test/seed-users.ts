...
async function mintIdToken(uid: string, phone: string): Promise<string> {
  ...
  if (!res.ok || !data.idToken) {
    throw new Error(
      `signInWithCustomToken failed for ${uid}: ${data.error?.message ?? res.status}`,
    );
  }
  return data.idToken;
}
...
async function establishSession(idToken: string): Promise<string> {
  ...
  try {
    data = JSON.parse(text) as { user?: { id: string } };
  } catch {
    throw new Error(`POST /v1/session non-JSON response (${res.status}): ${text.slice(0, 500)}`);
  }
  if (!res.ok || !data.user)
    throw new Error(`POST /v1/session failed (${res.status}): ${text.slice(0, 500)}`);
  return data.user.id;
}
