...
#
# Wired into the EC2 crontab to run every 30 minutes:
#   */30 * * * * /home/ec2-user/aroha-backend/scripts/cron-low-balance-alert.sh \
...
curl -fsS --max-time 30 -X POST \
  ...
  -d '{}' \
  "http://127.0.0.1:${PORT}/internal/cron/low-balance-alert" || CURL_EXIT=$?

if [ "$CURL_EXIT" -ne 0 ]; then
  echo "$(date -u +%FT%TZ) ERROR: cron-low-balance-alert.sh failed (curl exit $CURL_EXIT)" >&2
  if [ -n "$TELEGRAM_BOT_TOKEN" ] && [ -n "$TELEGRAM_ALERT_CHAT_ID" ]; then
    curl -fsS --max-time 10 -X POST \
      -H 'Content-Type: application/json' \
      -d "{\"chat_id\":\"${TELEGRAM_ALERT_CHAT_ID}\",\"text\":\"cron-low-balance-alert.sh failed (curl exit ${CURL_EXIT})\"}" \
