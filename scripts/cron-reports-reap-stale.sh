#!/usr/bin/env bash
#
# Reaps purchased-report rows stuck at 'generating' because the process that
# claimed them crashed mid-run (see REPORT_STALE_GENERATING_MS in
# reports.repo.ts and reapStaleReports in reports.service.ts).
#
# Wired into the EC2 crontab to run every 5 minutes:
#   */5 * * * * /home/ec2-user/aroha-backend/scripts/cron-reports-reap-stale.sh \
#     >> /home/ec2-user/cron-reports-reap-stale.log 2>&1
#
...
curl -fsS --max-time 30 -X POST \
  ...
  -d '{}' \
  "http://127.0.0.1:${PORT}/internal/cron/reports-reap-stale" || CURL_EXIT=$?

if [ "$CURL_EXIT" -ne 0 ]; then
  echo "$(date -u +%FT%TZ) ERROR: cron-reports-reap-stale.sh failed (curl exit $CURL_EXIT)" >&2
  if [ -n "$TELEGRAM_BOT_TOKEN" ] && [ -n "$TELEGRAM_ALERT_CHAT_ID" ]; then
    curl -fsS --max-time 10 -X POST \
      -H 'Content-Type: application/json' \
      -d "{\"chat_id\":\"${TELEGRAM_ALERT_CHAT_ID}\",\"text\":\"cron-reports-reap-stale.sh failed (curl exit ${CURL_EXIT})\"}" \
