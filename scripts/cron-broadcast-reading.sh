...
curl -fsS --max-time 3600 -X POST \
  ...
  -d '{}' \
  "http://127.0.0.1:${PORT}/internal/cron/broadcast-daily-reading" || CURL_EXIT=$?
echo

if [ "$CURL_EXIT" -ne 0 ]; then
  echo "$(date -u +%FT%TZ) ERROR: cron-broadcast-reading.sh failed (curl exit $CURL_EXIT)" >&2
  if [ -n "$TELEGRAM_BOT_TOKEN" ] && [ -n "$TELEGRAM_ALERT_CHAT_ID" ]; then
    curl -fsS --max-time 10 -X POST \
      -H 'Content-Type: application/json' \
      -d "{\"chat_id\":\"${TELEGRAM_ALERT_CHAT_ID}\",\"text\":\"cron-broadcast-reading.sh failed (curl exit ${CURL_EXIT})\"}" \
