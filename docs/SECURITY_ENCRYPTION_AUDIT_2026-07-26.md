...
- **Key-loss warning, verbatim from the file's own header comment (lines 8–11):** _"`ENCRYPTION_KEY` must be a 32-byte key, base64-encoded... Losing it means the encrypted columns become unrecoverable — back it up outside the app's own `.env` (e.g. a secrets manager), the same way `DATABASE_URL` and the Firebase Admin key are handled."_ This is a design property, not an oversight: there is no key-recovery mechanism anywhere in this codebase.
